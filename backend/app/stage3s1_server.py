...
app = FastAPI(title="DrosoMath telemetry API", version="0.14.0")
app.add_middleware(
    CORSMiddleware,
...
PROBE_EVERY = 20
TRAIN_TRIALS = 120_000
TRAINING_MILESTONES = (15_000, 40_000, 80_000, 120_000)
...
FINAL_CHECKPOINT = CHECKPOINT_DIR / "stage3s1_seed41_t120000_v1.npz"


def _milestone_checkpoint(trial: int) -> Path:
    return CHECKPOINT_DIR / f"stage3s1_seed41_t{trial}_v1.npz"
...
def _training_task(experiment: SymbolicStage3S1Experiment, trial: int) -> tuple[str, str]:
    if trial <= 15_000:
        return "identity", "identity_grounding_0_15k"
    if trial <= 40_000:
        task = "successor" if float(experiment.rng.random()) < 0.80 else "identity"
        return task, "successor_curriculum_15_40k"

    r = float(experiment.rng.random())
    if r < 0.80:
        return "addition", "addition_curriculum_40_120k"
    if r < 0.90:
        return "successor", "addition_curriculum_40_120k"
    return "identity", "addition_curriculum_40_120k"


def prepare_experiment() -> tuple[SymbolicStage3S1Experiment, dict[str, Any], dict[str, Any], str]:
    experiment = SymbolicStage3S1Experiment()
    ...
    overall = Metrics()
    task_metrics = {task: Metrics() for task in ("identity", "successor", "addition")}
    snapshots: dict[str, Any] = {}

    for trial in range(1, TRAIN_TRIALS + 1):
        task, curriculum_phase = _training_task(experiment, trial)
        is_probe = trial % PROBE_EVERY == 0
        result = experiment.step(
            trial,
            task=task,
            learn=not is_probe,
            pair_pool=ADDITION_TRAIN_PAIRS if task == "addition" else None,
        ...
        if trial in TRAINING_MILESTONES:
            ...
            snapshot["curriculum_phase"] = curriculum_phase
            snapshot["tasks"] = {name: meter.snapshot() for name, meter in task_metrics.items()}
            snapshot["action_gating"] = {
                "identity_successor_active_outputs": 10,
                "addition_active_outputs": 19,
            }
            snapshots[str(trial)] = snapshot
            metadata = {
                "stage": "3S.1_symbolic_pretraining",
    ...
    return experiment, snapshots[str(TRAIN_TRIALS)], snapshots, "deterministic_120k_stage3s1_curriculum"
...
def _profile_example(profile: str, index: int) -> tuple[str, int | None, int | None]:
    ...
    if profile == "addition_seen":
        ...
        pair = ADDITION_COMMUTATIVITY_PAIRS[index % len(ADDITION_COMMUTATIVITY_PAIRS)]
    elif profile == "addition_heldout_pairs":
        pair = ADDITION_HELD_OUT_PAIRS[index % len(ADDITION_HELD_OUT_PAIRS)]
    else:
        raise ValueError(f"Unknown Stage 3S.1 profile: {profile}")
...
def health() -> dict[str, Any]:
    return {
        "status": "ok",
        "experiment": "symbolic_stage3s1",
        "mode": "120k_masked_outputs_recurrent_internal_plasticity_then_frozen_generalization",
    ...
    logger = RunLogger(
        {
            ...
            "learning_model": "reward_gated_recurrent_symbolic_associator",
            "chance_accuracy": 1.0 / N_OUTPUTS,
            "layout_source": LAYOUT["source"],
            "layout_count": NEURON_COUNT,
            "evaluation_profile": "symbolic_frozen_generalization",
            ...
            "learner": learner,
            "scientific_scope": (
                "Stage 3S.1 keeps arbitrary symbol codes and no numeric magnitude features. Identity/successor use a "
                "0..9 action mask so arithmetic-only outputs 10..18 are not negatively trained before addition. A shared "
                "token projection and recurrent sparse state are reward-gated plastic. The target is used only after choice "
                "to issue scalar reward. Frozen tests still separate seen, reversed, and completely held-out addition pairs. "
                "FlyWire coordinates remain visualization-only, not simulated connectome spikes."
            ),
        }
    )
            ...
            while profile_trial <= PROFILE_TRIALS:
                ...
                for _ in range(TRIALS_PER_UI_FRAME):
                    ...
                    result = experiment.step(
                        ...
                        a=a,
                        b=b,
                        trial_kind="stage3s1_frozen_eval",
                    )
                    ...
                    frame = {
                        ...
                        "telemetry_source": "symbolic_math_prototype",
                        "activity_source": "display_proxy_not_connectome_spikes",
                        "phase": "stage3s1_symbolic_frozen_generalization",
