...
from .symbolic_stage3s import (
    ...
    N_OUTPUTS,
    SymbolicStage3SExperiment,
)


app = FastAPI(title="DrosoMath telemetry API", version="0.13.0")
app.add_middleware(
    CORSMiddleware,
...
FINAL_CHECKPOINT = CHECKPOINT_DIR / "stage3s_seed31_t120000_v1.npz"


def _milestone_checkpoint(trial: int) -> Path:
    return CHECKPOINT_DIR / f"stage3s_seed31_t{trial}_v1.npz"
...
def _training_task(experiment: SymbolicStage3SExperiment, trial: int) -> tuple[str, str]:
    if trial <= 15_000:
...
def prepare_experiment() -> tuple[SymbolicStage3SExperiment, dict[str, Any], dict[str, Any], str]:
    experiment = SymbolicStage3SExperiment()
    ...
    return experiment, snapshots[str(TRAIN_TRIALS)], snapshots, "deterministic_120k_symbolic_curriculum"
...
def _profile_example(profile: str, index: int) -> tuple[str, int | None, int | None]:
    ...
    if profile == "addition_seen":
        ...
        raise ValueError(f"Unknown Stage 3S profile: {profile}")
...
def health() -> dict[str, Any]:
    return {
        "status": "ok",
        "experiment": "symbolic_stage3s",
        "mode": "120k_symbol_grounding_successor_addition_then_frozen_generalization",
    ...
    logger = RunLogger(
        {
            ...
            "learning_model": "reward_modulated_sparse_symbol_associator",
            "chance_accuracy": 1.0 / N_OUTPUTS,
            ...
            "learner": learner,
            "scientific_scope": (
                "Stage 3S removes dot perception entirely. Inputs are arbitrary fixed sparse codes for visible symbols; "
                "no scalar magnitude, ordinal embedding, sum, answer hint, or distance-to-answer feature is supplied. "
                "The external program knows the answer only to issue reward after the learner chooses an action. Frozen "
                "tests separately measure seen addition pairs, reversed operand order, and unordered pairs never rewarded "
                "during addition training. FlyWire coordinates remain visualization-only."
            ),
        }
    )
            ...
            while profile_trial <= PROFILE_TRIALS:
                ...
                for _ in range(TRIALS_PER_UI_FRAME):
                    ...
                    result = experiment.step(
                        ...
                        a=a,
                        b=b,
                        trial_kind="stage3s_frozen_eval",
                    )
                    ...
                    frame = {
                        ...
                        "telemetry_source": "symbolic_math_prototype",
                        "activity_source": "display_proxy_not_connectome_spikes",
                        "phase": "stage3s_symbolic_frozen_generalization",
