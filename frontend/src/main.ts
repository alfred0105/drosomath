...
const regionBase: Record<string, THREE.Color> = {
  ...
  central: new THREE.Color('#7ea98a'),
...
function setNeuronColor(index: number, activity: number) {
  ...
  const base = regionBase[neuron.region] ?? new THREE.Color('#8590a0');
  const hot = new THREE.Color('#ffffff');
  const mixed = base.clone().lerp(hot, Math.min(1, activity));
...
async function loadLayout() {
  ...
  statusEl.textContent = `${data.count.toLocaleString()} neurons · ${data.source} layout`;
}
...
function connectTelemetry() {
  ...
  socket.addEventListener('open', () => {
    statusEl.textContent = `${neurons.length.toLocaleString()} neurons · live 10 Hz`;
  });
  ...
  socket.addEventListener('message', (event) => {
    ...
    for (const [id, activity] of frame.activity) {
      ...
      if (index !== undefined) setNeuronColor(index, activity);
    }
  ...
  socket.addEventListener('close', () => {
    statusEl.textContent = 'telemetry disconnected · retrying…';
...
function animate() {
  controls.update();
  if (pointCloud) pointCloud.rotation.y += 0.0005;
