...
from .curriculum_v2_memory import (
    MemoryV2Config,
    _make_memory_session,
    _phase1_gate,
)


DEFAULT_RESULT = Path("results/latest_malecns_v21_memory.json")
DEFAULT_PROGRESS = Path("results/malecns_v21_memory_progress.json")
DEFAULT_CHECKPOINT = Path("checkpoints/malecns_v21_memory_brain.npz")
DEFAULT_READOUT_DIR = Path("checkpoints/malecns_v21_memory_readouts")

Observer = Callable[[dict[str, object], PlasticMaleCNSBrain], None]


@dataclass(frozen=True, slots=True)
class MemoryV21Config(MemoryV2Config):
    adaptive_error_power: float = 2.0
    adaptive_min_weight: float = 0.03
    adaptive_ema_decay: float = 0.85
    consolidation_min_stage_gain: float = 1e-6
...
def run_memory_v21_curriculum(
    connectome,
    *,
    config: MemoryV21Config,
    ...
    """Memory v2.1: weak-task adaptive replay + stage-local consolidation."""
    np = __import__("numpy")
    ...
    for i, task in enumerate(tasks):
        readout, session = _make_memory_session(brain, output, task, config, config.seed + 300 + i)
    ...
    progress = _load(progress_path) if resume else None
    ...
    consolidator = MemoryConsolidator(
        ConsolidationConfig(
            ...
            min_stage_gain=config.consolidation_min_stage_gain,
        )
    )
    replay = AdaptiveReplayScheduler(
        AdaptiveReplayConfig(
            ...
            error_power=config.adaptive_error_power,
            min_weight=config.adaptive_min_weight,
            ema_decay=config.adaptive_ema_decay,
        )
    )
    ...
    for stage_index in range(start_stage, len(tasks)):
        ...
        emit(
            ...
            message=f"Memory v2.1 stage {stage_index+1}/{len(tasks)}: {task.name}",
        )
        ...
        # Seed replay priorities from the latest measured retention. A task that
        # is already near-perfect receives little replay; a weak task receives more.
        if retention_history:
            latest = retention_history[-1].get("tasks", {})
            for prior_index, prior in enumerate(tasks[:stage_index]):
                row = latest.get(prior.name)
                if row is not None:
                    replay.set_accuracy(prior_index, float(row["accuracy"]))

        # Snapshot immediately before CNS learning. Decoder/calibration/evaluation
        # run with plastic tracking disabled, so this isolates current-stage gains.
        stability_before_stage = brain.plasticity.stability.copy()
        ...
        replay_correct = {prior.name: 0 for prior in tasks[:stage_index]}
        ...
        for trial in range(start_trial, config.stage_trials):
            ...
            if replay.should_replay(trial + 1, stage_index):
                ...
                replay.update(prior_index, correct=bool(replay_result.correct))
                replay_counts[prior.name] += 1
                replay_correct[prior.name] += int(replay_result.correct)
                replay_rows.append(
                    {
            ...
            emit(
                ...
                replay_policy=replay.snapshot(stage_index),
                last_trial=row,
            )
            ...
            if config.checkpoint_every and (trial + 1) % config.checkpoint_every == 0:
                ...
                _write(
                    progress_path,
                    {
                        "experiment": "malecns_memory_v2_1_progress",
        ...
        replay_report = {
            ...
            "accuracy": (sum(int(x["correct"]) for x in replay_rows) / len(replay_rows)) if replay_rows else None,
            "silent_fraction": (sum(int(x["silent"]) for x in replay_rows) / len(replay_rows)) if replay_rows else None,
            "per_task": {
                name: {
                    "count": replay_counts[name],
                    "accuracy": replay_correct[name] / replay_counts[name] if replay_counts[name] else None,
                }
        ...
        for prior_index, prior in enumerate(tasks[: stage_index + 1]):
            ...
            replay.set_accuracy(prior_index, acc)
    ...
    result = {
        "experiment": "malecns_memory_v2_1",
